...
-- Latest Vehicle Status Table (for API 2 latest_status expansion)
CREATE TABLE IF NOT EXISTS latest_vehicle_status (
    vehicle_id UUID PRIMARY KEY REFERENCES vehicles(id) ON DELETE CASCADE,
    battery_level_percent DECIMAL(5, 2) CHECK (battery_level_percent >= 0 AND battery_level_percent <= 100),
    range_km DECIMAL(8, 2) CHECK (range_km >= 0),
...
CREATE INDEX IF NOT EXISTS idx_latest_status_recorded ON latest_vehicle_status(recorded_at DESC);
...
-- Create trigger for latest_vehicle_status updated_at
CREATE TRIGGER update_latest_status_updated_at
    BEFORE UPDATE ON latest_vehicle_status
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
